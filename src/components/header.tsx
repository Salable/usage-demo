...
export const Header = () => {
  ...
  const {data: session, mutate: mutateSession, isValidating: isValidatingSession, isLoading: isLoadingSession} = useSWR<Session>(`/api/session`)
  const {data: user, isLoading: isLoadingUser, isValidating: isValidatingUser} = useSWR<User>(`/api/users/${session?.uuid}`)
      ...
      <div className='max-w-[1000px] m-auto py-4 flex justify-between items-center'>
        ...
        <div>
          <div className="flex justify-between items-center">
            {(!isValidatingSession && !isLoadingSession) && (!isLoadingUser && !isValidatingUser) ? (
              <>
              ...
              <div className="animate-pulse">
                <div className="h-[38px] bg-slate-300 rounded-full w-[38px]"></div>
              </div>
            )}
          </div>
        </div>
